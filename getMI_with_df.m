...
% Defaults
if nargin < 5 || isempty(units), units = 'bits'; end
if nargin < 4 || isempty(smoothingValue), smoothingValue = 0.5; end
...
if istable(x)
    ...
    if nargin < 2 || isempty(y)
        error('getMI:MissingColumnNames', ...
              'When first input is a table, second input must be {''colX'',''colY''}.');
    end
    if ~iscell(y) || numel(y) ~= 2 || ~all(cellfun(@ischar, y))
        error('getMI:BadColumnNamesType', ...
              'For table input, use a 1x2 cell array of chars: {''x1'',''y1''}.');
    end
    if ~all(ismember(y, tbl.Properties.VariableNames))
        error('getMI:ColumnsNotFound', 'One or both specified columns not found in the table.');
    end
